...
import { 
  ...
  CheckCircle2, ExternalLink, Sparkles, Compass, Lightbulb, 
  BookmarkCheck, MonitorPlay, LayoutGrid, Download
} from 'lucide-react';
...
export const ResultView: React.FC<ResultViewProps> = ({ onBackToMap }) => {
  const { activeStudent, getStudentJourney, driveFolderUrl } = useApp();
  ...
  const handlePrintPdf = () => {
    // When printing, we print all 5 PPT 16:9 slides seamlessly
    window.print();
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Automatic Google Drive Synchronization Status Banner */}
      <div className="no-print p-4 bg-emerald-50 border border-emerald-200/80 rounded-2xl flex flex-col sm:flex-row items-center justify-between gap-3 text-xs">
        <div className="flex items-center gap-3 text-emerald-900">
          ...
          <div>
            <p className="font-extrabold text-xs">
              {isFinished ? 'Data Refleksi Otomatis Tersimpan ke Google Drive' : 'Penyimpanan Google Drive Otomatis Aktif'}
            </p>
            <p className="text-emerald-800 text-[11px] mt-0.5">
              Folder Tujuan BK: <span className="font-mono text-[10px] text-emerald-950 font-bold break-all">{driveFolderUrl}</span>
            </p>
          </div>
        </div>
        <a
          ...
          className="text-emerald-800 bg-white hover:bg-emerald-100/50 border border-emerald-300 font-bold px-3 py-1.5 rounded-xl text-xs flex items-center gap-1.5 shrink-0 transition-all"
        >
      ...
      <div className="space-y-8 print:space-y-0">
        ...
        {(viewMode === 'all' || currentSlide === 1) && (
          <div className="ppt-slide aspect-[16/9] w-full bg-gradient-to-br from-slate-900 via-teal-950 to-emerald-950 text-white p-8 sm:p-14 rounded-3xl shadow-xl flex flex-col justify-between relative overflow-hidden border border-slate-700 print:rounded-none print:shadow-none print:border-none print:aspect-[16/9] print:h-screen print:w-screen print:break-after-page">
            ...
            <div className="flex items-center justify-between relative z-10">
              <span className="px-4 py-1.5 rounded-full bg-white/10 backdrop-blur-md border border-white/20 text-teal-200 text-xs sm:text-sm font-bold tracking-widest uppercase">
            ...
            <div className="space-y-4 my-auto relative z-10 max-w-4xl">
              ...
              <p className="text-base sm:text-xl text-teal-100/90 font-medium leading-relaxed">
                Delapan pos untuk mengubah rasa ragu menjadi keberanian bertumbuh
              </p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-6 border-t border-white/15 relative z-10">
              <div className="bg-white/10 backdrop-blur-md p-4 rounded-2xl border border-white/15 space-y-1">
                ...
                <p className="text-xs text-teal-200">
                  Kelas: {activeStudent.class} • Tanggal: {new Date().toLocaleDateString('id-ID', { year: 'numeric', month: 'long', day: 'numeric' })}
                </p>
              </div>
              ...
              <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs space-y-3">
                ...
                <div className="space-y-1.5 text-xs">
                  <p className="font-bold text-emerald-800">
                    Tukar kalimat: dari "Saya tidak bisa" ➔ menjadi "Saya belum bisa":
                  </p>
                  <p className="bg-emerald-50/70 p-2.5 rounded-xl text-emerald-950 font-medium border border-emerald-200">
                    ...dan saya sedang belajar dengan cara: <span className="font-bold">{getStageAnswer(2, 'growth_learning_way') || 'Belum diisi'}</span>
                  </p>
                </div>
          ...
          <div className="ppt-slide aspect-[16/9] w-full bg-slate-50 p-6 sm:p-10 rounded-3xl shadow-xl flex flex-col justify-between border border-slate-200 print:rounded-none print:shadow-none print:border-none print:aspect-[16/9] print:h-screen print:w-screen print:break-after-page">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 my-auto">
              ...
              <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div className="bg-slate-50 p-2.5 rounded-xl border border-slate-100">
                    <p className="font-bold text-slate-700">Hambatan Internal (Diri):</p>
                    <p className="text-slate-800 mt-1">{getStageAnswer(3, 'internal_obstacles') || '-'}</p>
                  </div>
                  <div className="bg-slate-50 p-2.5 rounded-xl border border-slate-100">
                    <p className="font-bold text-slate-700">Hambatan Eksternal (Luar):</p>
                    <p className="text-slate-800 mt-1">{getStageAnswer(3, 'external_obstacles') || '-'}</p>
                  </div>
                </div>

                <div className="space-y-1 text-xs">
                  <p className="font-bold text-slate-700">Mengapa hambatan itu muncul?</p>
                  <p className="bg-slate-50 p-2 rounded-xl text-slate-800 border border-slate-100">
                    {getStageAnswer(3, 'why_obstacle_arises') || '-'}
                  </p>
                </div>

                <div className="space-y-1 text-xs">
                  <p className="font-bold text-emerald-800">Cara saya melewatinya:</p>
                  <p className="bg-emerald-50/70 p-2.5 rounded-xl text-emerald-950 font-semibold border border-emerald-200">
                    {getStageAnswer(3, 'how_to_overcome') || '-'}
                  </p>
                </div>

                <div className="bg-slate-50 p-2 rounded-xl text-xs border border-slate-100">
                  <p className="text-[10px] font-bold text-slate-500 uppercase">Kalimat saat ingin menyerah:</p>
                  <p className="font-bold text-slate-800 italic">"{getStageAnswer(3, 'giveup_motto') || '-'}"</p>
                </div>
              </div>
              ...
              <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs space-y-3">
                ...
                <div className="space-y-1.5 text-xs">
                  <p className="font-bold text-slate-700">Tiga langkah menuju tantangan saya:</p>
                  <div className="space-y-1">
                    <div className="bg-slate-50 p-2 rounded-xl border border-slate-100 flex items-center gap-2">
                      ...
                      <span className="text-slate-800 font-medium">{getStageAnswer(4, 'step_1') || '-'}</span>
                    </div>
                    <div className="bg-slate-50 p-2 rounded-xl border border-slate-100 flex items-center gap-2">
                      ...
                      <span className="text-slate-800 font-medium">{getStageAnswer(4, 'step_2') || '-'}</span>
                    </div>
                    <div className="bg-slate-50 p-2 rounded-xl border border-slate-100 flex items-center gap-2">
                      ...
                      <span className="text-slate-800 font-medium">{getStageAnswer(4, 'step_3') || '-'}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-2 text-xs">
                  <div className="bg-slate-50 p-2 rounded-xl border border-slate-100">
                    <p className="text-[10px] font-bold text-slate-400">Mulai Tanggal:</p>
                    <p className="text-slate-800 font-bold mt-0.5">{getStageAnswer(4, 'start_date') || '-'}</p>
                  </div>
                  <div className="bg-slate-50 p-2 rounded-xl border border-slate-100">
                    <p className="text-[10px] font-bold text-slate-400">Agar Konsisten:</p>
                    <p className="text-slate-800 font-medium line-clamp-2 mt-0.5">{getStageAnswer(4, 'consistency_strategy') || '-'}</p>
                  </div>
                  <div className="bg-slate-50 p-2 rounded-xl border border-slate-100">
                    <p className="text-[10px] font-bold text-slate-400">Yang Membantu:</p>
                    <p className="text-slate-800 font-medium line-clamp-2 mt-0.5">{getStageAnswer(4, 'helper_person') || '-'}</p>
                  </div>
                </div>

                <div className="bg-blue-50/60 p-2.5 rounded-xl text-xs border border-blue-100">
                  <p className="text-[10px] font-bold text-blue-900 uppercase">Kalimat setiap selesai satu langkah:</p>
                  <p className="font-bold text-blue-950 italic">"{getStageAnswer(4, 'step_done_motto') || '-'}"</p>
                </div>
              </div>
            </div>
            ...
            {/* Slide Footer */}
            <div className="text-[11px] text-slate-400 italic pt-2 flex justify-between border-t border-slate-200">
              <span>Siklus Pertemuan 1 selesai ➔ Mencoba langkah selama 1 minggu</span>
              <span>Growth Mindset Journey Map</span>
            </div>
          </div>
        )}
          ...
          <div className="ppt-slide aspect-[16/9] w-full bg-slate-50 p-6 sm:p-10 rounded-3xl shadow-xl flex flex-col justify-between border border-slate-200 print:rounded-none print:shadow-none print:border-none print:aspect-[16/9] print:h-screen print:w-screen print:break-after-page">
            ...
            <div className="flex items-center justify-between pb-3 border-b border-slate-200">
              <div className="flex items-center gap-3">
                <span className="px-3 py-1 rounded-xl bg-indigo-600 text-white font-black text-xs uppercase tracking-wider">
                  Etape 2 : Belajar Sekitar
                </span>
                <h2 className="text-base sm:text-lg font-black text-slate-800">
                  Evaluasi Kritik & Inspirasi Orang Lain
                </h2>
              </div>
              <span className="text-xs text-slate-400 font-mono">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 my-auto">
              ...
              <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs space-y-3">
                ...
                <div className="space-y-1 text-xs">
                  <p className="font-bold text-slate-700">Masukan yang pernah saya terima:</p>
                  <p className="bg-slate-50 p-2.5 rounded-xl text-slate-800 italic border border-slate-100">
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div className="bg-emerald-50/70 p-2 rounded-xl border border-emerald-200">
                    <p className="font-bold text-emerald-900">Membangun:</p>
                    <p className="text-emerald-950 mt-0.5">{getStageAnswer(5, 'constructive_aspect') || '-'}</p>
                  </div>
                  <div className="bg-red-50/70 p-2 rounded-xl border border-red-200">
                    <p className="font-bold text-red-900">Menjatuhkan:</p>
                    <p className="text-red-950 mt-0.5">{getStageAnswer(5, 'destructive_aspect') || '-'}</p>
                  </div>
                </div>

                <div className="space-y-1 text-xs">
                  <p className="font-bold text-slate-700">Yang saya ambil dan saya perbaiki:</p>
                  <p className="bg-slate-50 p-2 rounded-xl text-slate-800 font-medium border border-slate-100">
                    {getStageAnswer(5, 'what_i_improve') || '-'}
                  </p>
                </div>

                <div className="bg-violet-50/70 p-2.5 rounded-xl text-xs border border-violet-100">
                  <p className="text-[10px] font-bold text-violet-900 uppercase">Mulai sekarang merespons kritik dengan cara:</p>
                  <p className="font-bold text-violet-950 mt-0.5">{getStageAnswer(5, 'response_strategy') || '-'}</p>
                </div>
              </div>
              ...
              <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs space-y-3">
                <div className="flex items-center justify-between border-b border-indigo-100 pb-2">
                  <h3 className="font-extrabold text-sm text-indigo-800 flex items-center gap-2">
                ...
                <div className="space-y-1 text-xs">
                  <p className="font-bold text-slate-700">Sosok yang dikagumi keberaniannya & siapa dia:</p>
                  <p className="bg-slate-50 p-2.5 rounded-xl text-slate-800 font-bold border border-slate-100">
                ...
                <div className="space-y-1 text-xs">
                  ...
                  <p className="bg-slate-50 p-2.5 rounded-xl text-slate-800 border border-slate-100">
                    {getStageAnswer(6, 'confidence_actions') || '-'}
                  </p>
                </div>

                <div className="space-y-1 text-xs">
                  <p className="font-bold text-indigo-800">Satu hal darinya yang bisa saya tiru minggu ini:</p>
                  <p className="bg-indigo-50/80 p-2.5 rounded-xl text-indigo-950 font-semibold border border-indigo-200">
                ...
                <div className="p-2.5 bg-slate-50 rounded-xl border border-slate-200 text-xs italic text-slate-600">
                  <span className="font-bold text-slate-700">Ingat: </span>
        ...
        {/* ========================================================================= */}
        {/* SLIDE 5: ETAPE 2 (POS 7 & POS 8) + KOMITMEN AKHIR */}
        {/* ========================================================================= */}
          ...
          <div className="ppt-slide aspect-[16/9] w-full bg-slate-50 p-6 sm:p-10 rounded-3xl shadow-xl flex flex-col justify-between border border-slate-200 print:rounded-none print:shadow-none print:border-none print:aspect-[16/9] print:h-screen print:w-screen print:break-after-page">
            ...
            <div className="flex items-center justify-between pb-3 border-b border-slate-200">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-base sm:text-lg font-black text-slate-800">
                  Refleksi Usaha & Komitmen Target
                </h2>
              </div>
              <span className="text-xs text-slate-400 font-mono">
              ...
              <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs space-y-3">
                <div className="flex items-center justify-between border-b border-purple-100 pb-2">
                  <h3 className="font-extrabold text-sm text-purple-800 flex items-center gap-2">
                    <span className="w-6 h-6 rounded-lg bg-purple-600 text-white flex items-center justify-center text-xs">7</span>
                ...
                <div className="space-y-1 text-xs">
                  <p className="font-bold text-emerald-800">Setelah mencoba langkah, apa yang sudah berhasil?</p>
                  <p className="bg-emerald-50/70 p-2.5 rounded-xl text-emerald-950 font-semibold border border-emerald-100">
                    {getStageAnswer(7, 'what_succeeded') || 'Belum diisi'}
                  </p>
                </div>

                <div className="space-y-1 text-xs">
                  <p className="font-bold text-slate-700">Bagian yang masih kurang berhasil & alasannya:</p>
                  <p className="bg-slate-50 p-2.5 rounded-xl text-slate-800 border border-slate-100">
                    {getStageAnswer(7, 'what_failed_and_why') || '-'}
                  </p>
                </div>

                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div className="bg-slate-50 p-2 rounded-xl border border-slate-100">
                    <p className="text-[10px] font-bold text-slate-500">Hal Baru Tentang Diri:</p>
                    <p className="text-slate-800 mt-0.5">{getStageAnswer(7, 'new_self_knowledge') || '-'}</p>
                  </div>
                  <div className="bg-slate-50 p-2 rounded-xl border border-slate-100">
                    <p className="text-[10px] font-bold text-slate-500">Perubahan Kecil Terasa:</p>
                    <p className="text-slate-800 mt-0.5">{getStageAnswer(7, 'felt_changes') || '-'}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs space-y-3">
                <div className="flex items-center justify-between border-b border-amber-100 pb-2">
                  <h3 className="font-extrabold text-sm text-amber-800 flex items-center gap-2">
                    <span className="w-6 h-6 rounded-lg bg-amber-600 text-white flex items-center justify-center text-xs">8</span>
                    <span>Komitmen dan Target Saya (Garis Akhir)</span>
                  </h3>
                ...
                <div className="space-y-1 text-xs">
                  <p className="font-bold text-slate-700">Target satu minggu ke depan:</p>
                  <div className="space-y-1">
                    <div className="bg-slate-50 p-1.5 px-2.5 rounded-lg border border-slate-100 flex items-center gap-2">
                      <span className="text-emerald-600 font-bold">☑</span>
                      <span className="text-slate-800 font-medium">{getStageAnswer(8, 'target_week_1') || '-'}</span>
                    </div>
                    <div className="bg-slate-50 p-1.5 px-2.5 rounded-lg border border-slate-100 flex items-center gap-2">
                      <span className="text-emerald-600 font-bold">☑</span>
                      <span className="text-slate-800 font-medium">{getStageAnswer(8, 'target_week_2') || '-'}</span>
                    </div>
                    <div className="bg-slate-50 p-1.5 px-2.5 rounded-lg border border-slate-100 flex items-center gap-2">
                      <span className="text-emerald-600 font-bold">☑</span>
                      <span className="text-slate-800 font-medium">{getStageAnswer(8, 'target_week_3') || '-'}</span>
                    </div>
                  </div>
                </div>

                <div className="pt-0.5 flex items-center justify-between text-xs bg-amber-50/50 p-2 rounded-xl border border-amber-100">
                  <span className="font-bold text-amber-900">Keyakinan perubahan diri:</span>
                  <div className="flex items-center gap-1 font-bold">
                    {[1, 2, 3, 4, 5].map((n) => (
                      <span
                        key={n}
                ...
                <div className="space-y-1 text-xs">
                  <p className="font-bold text-emerald-800">Saya berkomitmen untuk:</p>
                  <p className="bg-emerald-50/80 p-2.5 rounded-xl text-emerald-950 font-bold border border-emerald-200">
            ...
            {/* Slide Footer with Signature & Quote */}
            <div className="bg-white p-3 rounded-2xl border border-slate-200 flex items-center justify-between gap-4 text-xs">
              <div className="italic text-slate-600 max-w-xl">
                <span className="font-bold text-slate-700">Pegangan: </span>
                "{PEGANGAN_DI_SEPANJANG_JALAN}"
              </div>
              <div className="text-right shrink-0">
                <p className="text-[10px] text-slate-400 font-bold">Tanda Tangan Siswa</p>
                <p className="font-black text-slate-800 border-b border-slate-300 pb-0.5 min-w-[120px]">
                  {activeStudent.name}
                </p>
              </div>
            </div>
